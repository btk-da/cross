...
class Margin_account():
    ...
    def create_loan(self, order_qty, precision, asset, symbol):
        ...
        try:
            max_loan = float(self.client.get_max_margin_loan(asset=asset)['amount'])
            order_ok = True
        except:
            max_loan = 0
            order_ok = False
        ...
        if max_loan > loan_qty and order_ok:
            ...
            try:
                self.client.create_margin_loan(asset=asset, amount=loan_qty)
                order_ok = True
            except Exception as e:
                order_ok = False
    ...
    def repay_loan(self, symbol, amount, price, coin):
        ...
        if self.loans[symbol.asset] > 0 and self.balances[symbol.asset] > 0 and coin == 'Asset':
            ...
            self.client.repay_margin_loan(asset=symbol.asset, amount=loan_qty)
        ...
        if self.loans[self.base_coin] > 0 and self.balances[self.base_coin] > 0 and coin == 'Base':
            ...
            self.client.repay_margin_loan(asset=self.base_coin, amount=loan_qty)
            print('Loan Repay Filled', self.base_coin)     
            self.notifier.register_output('Action', symbol.asset, symbol.side, 'Loan Repaid')
    ...
    def extra_repay(self):
        
        try:                
            for item in self.client.get_margin_account()['userAssets']:
                if float(item['free']) > 0 and float(item['borrowed']) > 0:
                    self.client.repay_margin_loan(asset=item['asset'], amount=float(item['free']))
                    self.notifier.register_output('Info', item['asset'], 'general', 'Extra loan repay: ' + item['asset'])
        except Exception as e:
            print(f"Extra repay failed: {e}")
            traceback.print_exc()
            self.notifier.register_output('Error', 'general', 'general', 'Extra loan repay failed: ' + str(e))
        return
    ...
    def check_open_orders(self, time):
        ...
        for i in self.open_order_list:
            ...
            if open_order['status'] == 'FILLED':
                ...
                if symbol.timer > 5:
                    cancel_order = self.client.cancel_margin_order(symbol=symbol.tic, orderId=order['orderId'])
                ...
                if symbol.timer > 5:
                    self.client.cancel_margin_order(symbol=symbol.tic, orderId=open_order['orderId'])
                    self.notifier.send_cancel_order(symbol.asset, symbol.side, action, str(open_order['orderId']))
                    self.notifier.register_output('Cancel Order', symbol.asset, symbol.side, open_order['symbol'] + symbol.side + action + str(open_order['orderId']))
                    print('Cancel Order', open_order['symbol'] + symbol.side + action + str(open_order['orderId']))
                    symbol.timer = 0
    ...
    def create_buy_order(self, symbol, buy_amount, price, action):
        ...
        if self.balances[self.base_coin] >= order_qty * price:
            ...
            try:
                buy_open_order = self.client.create_margin_order(symbol=symbol.tic, side='BUY', type='LIMIT', timeInForce='GTC', quantity=order_qty, price=order_price)
    ...
    def create_sell_order(self, symbol, buy_amount, price, action):
        ...
        if self.balances[symbol.asset] >= order_qty:
            time = datetime(datetime.now().year, datetime.now().month, datetime.now().day, datetime.now().hour, datetime.now().minute, datetime.now().second)
            try:
                sell_open_order = self.client.create_margin_order(symbol=symbol.tic, side='SELL', type='LIMIT', timeInForce='GTC', quantity=order_qty, price=order_price)
    ...
    def calculate_nav(self, time):
        try:
            ...
            #self.nav_list = np.append(self.nav_list, [self.nav])
            #self.nav_time = np.append(self.nav_time, [time])
            #self.margin_list.append(self.margin)

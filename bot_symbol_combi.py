...
class Symbol_combi(object):
    ...
    def calculate_ratios(self, time):
        
        restart_ratios = delete(self.account.notifier.tables['ponderation'])
        sql_session.execute(restart_ratios)
        sql_session.commit()
            
        for asset in self.wr_list:
            new_row = self.account.notifier.tables['ponderation'](Date=str(time), Asset=asset[8:], Long_ratio=self.wr_list[asset]['Long'], Short_ratio=self.wr_list[asset]['Short'])
    ...
    def add_symbols(self, symbols):
        ...
        for params in symbols:
            if params['drop'] > 0:
                ...
                # print(str(self.wr_list[symbol.nick]))
    ...
    def init_params(self, backup): 
        ...
        self.account.indiv_max_leverage_funds = self.account.max_leverage_funds/26
